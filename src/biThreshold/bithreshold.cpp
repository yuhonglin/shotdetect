...
BiThreshold::BiThreshold( char** argv, int argc) : Algorithm( argv, argc, "biThreshold")
{
  ...
  op.add_float( KEY_HEIGHT_MARGIN_RATE, "height margin rate", 0.05, set<float>() );
  ...
  video->setROI(w*widthMarginRate, h*heightMarginRate, w*(1-2*widthMarginRate), h*(1-2*heightMarginRate));
...
bool BiThreshold::detect()
{
  ...
  while(true){
    ...
    if(midiff<lowThreshold){
      if(i - begin<minShotLength) continue;				     	         
      /* cut detected */
      shotBoundary.push_back( pair<float, float>(begin+margin, i-timeStep+margin) );
      begin = i;
      continue;						     	         
    }									     	         
	...
	midiff = compare(&cache, &foo);
	  ...
	  shotBoundary.push_back( pair<float, float>(begin+margin, i-margin) );
  	  i = j;							     	         
  	  begin = i;								     	         
  	  break;								     	         
  	}else{	  
